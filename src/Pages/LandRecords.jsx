...
// /*  (produces a real, valid single-page PDF file client-side)         */
// /* ------------------------------------------------------------------ */
...
//   function loadDemo(record, tab) {
//     setError('');
//     setSearchType('Khasra Number');
//     setQueryValue(record.khasra);
...
//     () => [
//       { icon: FileText, title: 'View Property Details', action: () => loadDemo(RECORDS[0], 'Overview') },
...
//       { icon: FileCheck2, title: 'Property Verification', action: () => { loadDemo(RECORDS[0], 'Overview'); setVerifyOpen(true); } },
...
//               {/* Demo shortcuts so the search is easy to try */}
//               <div className="flex flex-wrap items-center gap-2 mt-4">
//                 <span className="text-xs text-slate-400">Try a demo record:</span>
//                 {RECORDS.map((r) => (
//                   <button
//                     type="button"
//                     key={r.id}
//                     onClick={() => loadDemo(r, 'Overview')}
...
//                     Village: {activeRecord.village} • Tehsil: {activeRecord.tehsil} • District: {activeRecord.district}
//                   </p>
//                 </div>
...
export default function LandRecords() {
  ...
  // `type` lets a demo shortcut pre-select which search method it is demonstrating
  // (e.g. 'ULPIN/Parcel ID' vs 'Khasra Number') so the form reflects how the
